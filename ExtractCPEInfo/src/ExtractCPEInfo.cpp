...
class testExtractCPEInfo : public edm::stream::EDAnalyzer<edm::GlobalCache<CacheData>> {
public:
	explicit testExtractCPEInfo(const edm::ParameterSet&, const CacheData*);
	~testExtractCPEInfo(){};
...
std::unique_ptr<CacheData> testExtractCPEInfo::initializeGlobalCache(const edm::ParameterSet& config) 
{
	CacheData* cacheData = new CacheData();
	return std::unique_ptr<CacheData>(cacheData);
}

void testExtractCPEInfo::globalEndJob(const CacheData* cacheData) {
	printf("in global end job\n");

}
void testExtractCPEInfo::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
	// defining this function will lead to a *_cfi file being generated when compiling
	edm::ParameterSetDescription desc;
	descriptions.addWithDefaultLabel(desc);
}

void testExtractCPEInfo::ResetVars(){
    Index_layer = 0;
    Index_track = 0; 
    CotAlpha = 0.f;
    CotBeta = 0.f;
...
testExtractCPEInfo::testExtractCPEInfo(const edm::ParameterSet& config, const CacheData* cacheData)
...
void testExtractCPEInfo::beginJob() {

}

void testExtractCPEInfo::endJob() {
	printf("in end job\n");

}
void testExtractCPEInfo::endRun() {
    out_File->cd();
    out_Tree->Write();
    out_File->Close();
	printf("in end run\n");
...
void testExtractCPEInfo::analyze(const edm::Event& event, const edm::EventSetup& setup) {
...
DEFINE_FWK_MODULE(testExtractCPEInfo);
