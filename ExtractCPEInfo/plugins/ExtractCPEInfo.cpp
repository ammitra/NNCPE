...
private:
	...
	static const int SIMHITPERCLMAX = 10;             // max number of simhits associated with a cluster/rechit
        ...
        // BPIX
        int Index_layer;        // increasing r
        int Index_ladder;       // increasing phi
        int Index_module;       // increasing z (this value is always 1 for FPIX, so only track it for BPIX)

        // FPIX
        int Index_side;         // FPIX- or FPIX+
        int Index_disk;         // increasing abs(z)
        int Index_blade;        // increasing phi and r
        int Index_panel;        // fwd/backward
...
void ExtractCPEInfo::ResetVars(){
    Index_ladder = 0;       // increasing phi
    Index_module = 0;       // increasing z (this value is always 1 for FPIX, so only track it for BPIX)
    Index_side   = 0;       // FPIX- or FPIX+
    Index_disk   = 0;       // increasing abs(z)
    Index_blade  = 0;       // increasing phi and r
    Index_panel  = 0;       // fwd/backward
...
trackerHitAssociatorConfig_(config, consumesCollector()) {
    ...
    out_Tree->Branch("Index_layer", &Index_layer, "Index_layer/I");
    out_Tree->Branch("Index_track", &Index_track, "Index_track/I");
    out_Tree->Branch("Index_ladder", &Index_ladder, "Index_ladder/I");
    out_Tree->Branch("Index_module", &Index_module, "Index_module/I");
    out_Tree->Branch("Index_side", &Index_side, "Index_side/I");
    out_Tree->Branch("Index_disk", &Index_disk, "Index_disk/I");
    out_Tree->Branch("Index_blade", &Index_blade, "Index_blade/I");
    out_Tree->Branch("Index_panel", &Index_panel, "Index_panel/I");
    ...
    out_Tree->Branch("TYSIZE", &Ty_size, "TYSIZE/I");
    out_Tree->Branch("Cluster", &Cluster, "Cluster[TXSIZE][TYSIZE]/F");
		...
		for (unsigned int h = 0; h < track.recHitsSize(); h++) {
			...
			auto subdetid = (id.subdetId());
                        ...
                        if (subdetid == PixelSubdetector::PixelBarrel) { //&& subdetid != PixelSubdetector::PixelEndcap)
                            Index_layer  = tkTpl.pxbLayer(hit_detId);
                            Index_ladder = tkTpl.pxbLadder(hit_detId);
                            Index_module = tkTpl.pxbModule(hit_detId);
                            Index_side   = -1;
                            Index_disk   = -1;
                            Index_blade  = -1;
                            Index_panel  = -1;
                            ...
                            Index_ladder = -1;
                            Index_module = -1;
                            Index_side   = tkTpl.pxfSide(hit_detId);
                            Index_disk   = tkTpl.pxfDisk(hit_detId);
                            Index_blade  = tkTpl.pxfBlade(hit_detId);
                            Index_panel  = tkTpl.pxfPanel(hit_detId);
                        }
                        else {
                            continue;
                        }            
